...
export default function CategoryDeleteBtn({ info, user, update, setPages }) {
  const { confirm } = Modal;

  const success = () => {
    message.success("You Deleted Category");
  };
  ...
  const options = {
    method: "delete",
    url: `${process.env.REACT_APP_API_BASEURL}/api/admin/categories/${info.id}`,
    headers: {

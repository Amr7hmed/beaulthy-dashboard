...
export default function DeleteDistrictBtn({ info, user, update, setPages }) {
  const { confirm } = Modal;

  const success = () => {
    message.success("You Deleted Districts");
  };
  ...
  const options = {
    method: "delete",
    url: `${process.env.REACT_APP_API_BASEURL}/api/admin/districts/${info.id}`,
    headers: {
  ...
  const showDeleteConfirm = function () {
    confirm({
      title: "Are you sure delete this Districts ?",
